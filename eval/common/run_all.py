...
def train_mt(args, q, index, cpu_opt, num_devices):
    while True:
        item = q.get()
        if item is None:
            break
        ...
        cmd = f'CUDA_VISIBLE_DEVICES={index % num_devices} taskset -c {cpu_opt} python setup.py \
                --content {content} --mode {args.mode} \
...
if __name__ == '__main__':
    ...
    for i in range(num_workers):
        ...
        t = threading.Thread(target=train_mt, args=(args, q, i, cpu_opt, num_devices))
        t.start()
