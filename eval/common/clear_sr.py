...
if __name__ == '__main__':
    ...
    # remove sr raw frames
    sr_image_dir = os.path.join(args.data_dir, args.content, 'image', lr_video_name, model.name)
    for f in os.listdir(sr_image_dir):
        f = os.path.join(sr_image_dir, f)
        if os.path.isfile(f):
            os.remove(os.path.join(sr_image_dir, f))

    # remove sr ppm images
