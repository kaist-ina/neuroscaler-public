...
def train_mt(args_, q, index, cpu_opt):
    while True:
        ...
        cmd = f'taskset -c {cpu_opt} python uniform_selector.py --content {content} --avg_anchors 12'
        os.system(cmd)
        cmd = f'taskset -c {cpu_opt} python uniform_selector.py --content {content} --avg_anchors 18'
        os.system(cmd)
        cmd = f'taskset -c {cpu_opt} python encode_selective.py --content {content} --avg_anchors 12'
        os.system(cmd)
        cmd = f'taskset -c {cpu_opt} python encode_selective.py --content {content} --avg_anchors 18'
        os.system(cmd)
        end = time.time()
