...
def train_mt(args_, q, index, cpu_opt):
    while True:
        ...
        for i in range(1, 31):
            start = time.time()
            cmd = f'taskset -c {cpu_opt} python engorgio_selector.py --content {content} --avg_anchors {i}'
            os.system(cmd)
            cmd = f'taskset -c {cpu_opt} python encode_engorgio.py --content {content} --avg_anchors {i}'
            os.system(cmd)
            end = time.time()
            print(f'{content}, {i}: {end - start}sec')
...
if __name__ == '__main__':
    ...
    q = queue.Queue()
    threads = []
    num_threads = len(common.contents)
