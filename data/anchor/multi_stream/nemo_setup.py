...
class Evaluator:
    def __init__(self, args):
        ...
        # codec
        self.num_decoders = 8
        self.gop = self.video_dataset.gop
        self.skip = 0
    ...
    def _setup_bilinear_quality(self, content):
        ...
        quality_bilinear = libvpx.bilinear_quality(self.vpxdec_path, content_dir, lr_video_name, hr_video_name, self.video_dataset.output_width, self.video_dataset.output_height)
    ...
    def _setup_anchor_quality(self, content):
        ...
        num_chunks = int(math.ceil(self.total_length / self.gop))
        for chunk_idx in range(num_chunks):
            ...
            libvpx.save_yuv_frame(self.vpxdec_path, content_dir, hr_video_name, self.video_dataset.output_width, self.video_dataset.output_height, num_skipped_frames, num_decoded_frames, postfix)
            ...
            quality_dnn = libvpx.offline_dnn_quality(self.vpxdec_path, content_dir, lr_video_name, hr_video_name, model.name, \
                                                    self.video_dataset.output_width, self.video_dataset.output_height, num_skipped_frames, num_decoded_frames, postfix)
            ...
            decoders = [mp.Process(target=libvpx.offline_cache_quality_mt, args=(q0, q1, self.vpxdec_path, content_dir, \
                                        lr_video_name, hr_video_name, model.name, self.video_dataset.output_width, self.video_dataset.output_height)) for i in range(self.num_decoders)]
