...
_set_conda
_set_env
CUDA_VISIBLE_DEVICES=${gpu_index} python ${ENGORGIO_CODE_ROOT}/engorgio/dnn/train_video.py \
        --data_dir /workspace/research/engorgio/dataset --result_dir /workspace/research/engorgio/result \
        --content ${content} --lr ${low_resolution} --hr ${high_resolution} --num_blocks ${num_blocks} --num_channels ${num_channels} --scale ${scale} --load_on_memory --model_name ${model_name}
